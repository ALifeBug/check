...
public class CourseController {
	...
	@RequestMapping("/getByCnameAndTno")
	protected JSONObject getByCnameAndTno(@RequestBody JSONObject jsonObject) throws IOException {
		if(jsonObject.isEmpty() || MjStringUtil.isEmpty(jsonObject.getString("courseName")) ||  MjStringUtil.isEmpty(jsonObject.getIntValue("tno")) ){
			return RespMsgUtil.getFailResponseJoErrJSONError();
		}
		Course course;
		try {
			course = courseRepository.findByCourseNameAndTno(jsonObject.getString("courseName"),jsonObject.getIntValue("tno"));
		} catch (Exception e) {
			e.printStackTrace();
			return RespMsgUtil.getFailResponseJoErrHibernate();
		}
